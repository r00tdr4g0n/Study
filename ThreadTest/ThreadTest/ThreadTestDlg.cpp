...
BEGIN_MESSAGE_MAP(CThreadTestDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CThreadTestDlg::OnBnClickedButtonStop)
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
UINT NumThread(LPVOID param)
{
	CThreadTestDlg* pMain = (CThreadTestDlg*)param;

	while (true) {
		if (WaitForSingleObject(pMain->m_pEvent, 10) != WAIT_OBJECT_0) {
			pMain->m_num++;
			pMain->m_strNum.Format(L"%d", pMain->m_num);
			pMain->SetDlgItemTextW(IDC_EDIT_NUMBER, pMain->m_strNum);
		}
	}
...
void CThreadTestDlg::OnBnClickedButtonStart()
{
	...
	if (m_pEvent == NULL) {
		m_pEvent = CreateEvent(NULL, TRUE, FALSE, L"kill");
		m_pThread = AfxBeginThread(NumThread, this);
	}
	else {
		ResetEvent(m_pEvent);
	}
...
void CThreadTestDlg::OnBnClickedButtonStop()
{
	// TODO: Add your control notification handler code here
